...
module.exports = {
    ...
    async execute(message, args) {
        ...
        async function loop(user, page, sort, currentMessage) {
                            ...
                            if (Array.isArray(value)) {
                                garage = garage.filter(function(car) {
                                    ...
                                    if (Array.isArray(currentCar[key])) {
                                        ...
                                        return value.every(tagFilter => {
                                            return obj[tagFilter] !== undefined
                                        });
                                    } else {
            ...
            if (lists.garageList.length > 1024) {
                const errorScreen = new Discord.MessageEmbed()
                    .setColor("#fc0303")
                    .setAuthor(message.author.tag, message.author.displayAvatarURL({
                        format: "png",
                    ...
                    .addField("Total Characters in List", `\`${lists.garageList.length}\` > \`1024\``)
                    .setTimestamp();
                return message.channel.send(errorScreen);
            }
            ...
            let garageMessage;
            if (currentMessage) {
                garageMessage = await currentMessage.edit({
                    embed: infoScreen,
                    component: row
                });
            } else {
                garageMessage = await message.channel.send({
                    embed: infoScreen,
                    component: row
                });
            }

            const collector = garageMessage.createButtonCollector(filter, {
                time: 60000
            });
