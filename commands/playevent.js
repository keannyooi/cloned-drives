...
module.exports = {
    ...
    async execute(message, args) {
        ...
        async function playEvent(event, currentMessage) {
            ...
            let test = require(`./cars/${player.carFile}`),
                passed = true;
            ...
            if (event.isActive || message.member.roles.cache.has("802043346951340064")) {
                ...
                const opponent = {
                    carFile: event.roster[round - 1].car,
                    gearingUpgrade: event.roster[round - 1].gearingUpgrade,
                    engineUpgrade: event.roster[round - 1].engineUpgrade,
                    chassisUpgrade: event.roster[round - 1].chassisUpgrade
                };
                ...
                const collector = reactionMessage.createButtonCollector(filter, {
                    time: 10000
                });
