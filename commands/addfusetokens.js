        ...
        if (message.mentions.users.first()) {
            ...
            new Promise(resolve => resolve(searchUser(message, args[0].toLowerCase(), availableUsers)))
                .then(async (hmm) => {
                    ...
                    await profileModel.updateOne({ userID: result.user.id }, { fuseTokens: balance });

                    const successMessage = new SuccessMessage({
                        channel: message.channel,
                        title: `Successfully added ${fuseEmoji}${amount} to ${result.user.username}'s fuse token balance!`,
